#ifndef PACKED_VECTOR
#define PACKED_VECTOR
...
explicit PackedVector(size_t elemSize, size_t initialSize = 0) :
    ...
    size_(initialSize),
    capacity_((initialSize*elemSize + WORD_BITS-1) / WORD_BITS)
{
    if (capacity_ > 0) {
...
size_t capacity() const {
    assert((elemSize == 0 && capacity_ == 0) || elemSize > 0);
